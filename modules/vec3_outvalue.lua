...
--- Clone a vector.
-- @tparam vec3 out Vector to clone into
-- @tparam vec3 other Vector to be cloned
-- @treturn vec3 out
function vec3.clone(out, other)
  out.x = other.x
  out.y = other.y
  out.z = other.z
  return out
...
--- Add two vectors.
-- @tparam vec3 out Vector container for the result
...
function vec3.trim(out, a, len)
  return out:normalize(a):scale(out, math.min(a:len(), len))
end
...
function vec3.lerp(out, a, b, s)
  return out:add(a, out:sub(b, a):scale(out, s)) -- a + (b - a) * s
end
